...
$(document).ready(function(){
	...
	$(document).on('change','#group_by_order_item',function(){
		...
		$('#order_holder .single_order').each(function(i, obj) {
			var $this = $(this);
			var found = 0;
			$(this).find(".items_holder .single_item").each(function(){
		        var this_menu_name = $(this).find('.item_name').html();
	...
	$('#order_holder.order_holder .single_order .items_holder').slimscroll({
		height: '199px'
	}).parent().css({
		background: '#fff',
		border: '0px solid #184055'
	...
	$(document).on('click','.items_holder .single_item',function(){
		...
		if(single_order.attr('data-order-type')=="Dine In"){
			if($(this).attr('data-selected')=="selected"){
				...
				if($(this).attr('data-cooking-status')=="Done" || $(this).attr('data-cooking-status')=="Started Preparing"){
	...
	$(document).on('click','.select_all_of_an_order',function(){
		...
		$('#single_order_'+order_id+' .items_holder .single_item').attr('data-selected','selected');
		$('#single_order_'+order_id+' .items_holder .single_item').addClass('light-sky-background');
		$('#single_order_'+order_id+' .items_holder .single_item').find('.item_quanity_text').css('color','#fff');
		$('#single_order_'+order_id+' .items_holder .single_item').find('.item_name').css('color','#fff');
		$('#single_order_'+order_id+' .items_holder .single_item').find('.item_qty').css('color','#fff');
		$('#single_order_'+order_id+' .items_holder .single_item').find('.modifiers').css('color','#fff');
		$('#single_order_'+order_id+' .items_holder .single_item').find('.note').css('color','#fff');
		$('#single_order_'+order_id+' .items_holder .single_item').attr('data-selected','selected');
	...
	$(document).on('click','.unselect_all_of_an_order',function(){
		...
		$('#single_order_'+order_id+' .items_holder .single_item').find('.item_qty').css('color','');
		$('#single_order_'+order_id+' .items_holder .single_item').find('.modifiers').css('color','');
		$('#single_order_'+order_id+' .items_holder .single_item').find('.note').css('color','');

		$('#single_order_'+order_id+' .items_holder .single_item[data-cooking-status="Done"]').find('.item_quanity_text').css('color','#fff');
		$('#single_order_'+order_id+' .items_holder .single_item[data-cooking-status="Done"]').find('.item_name').css('color','#fff');
		...
		$('#single_order_'+order_id+' .items_holder .single_item[data-cooking-status="Started Preparing"]').find('.item_quanity_text').css('color','#fff');
		$('#single_order_'+order_id+' .items_holder .single_item[data-cooking-status="Started Preparing"]').find('.item_name').css('color','#fff');
		$('#single_order_'+order_id+' .items_holder .single_item[data-cooking-status="Started Preparing"]').find('.item_qty').css('color','#fff');
		$('#single_order_'+order_id+' .items_holder .single_item[data-cooking-status="Started Preparing"]').find('.modifiers').css('color','#fff');
		$('#single_order_'+order_id+' .items_holder .single_item[data-cooking-status="Started Preparing"]').find('.note').css('color','#fff');
		$('#single_order_'+order_id+' .items_holder .single_item[data-cooking-status="Started Preparing"]').attr('data-selected','selected');	
		$('#start_cooking_button_'+order_id).fadeOut();
		$('#done_cooking_'+order_id).fadeOut();
	});

	$(document).on('click','.start_cooking_button',function(){
		var sale_id = $(this).attr('id').substr(21);
		if($('#single_order_'+sale_id).attr('data-order-type')=='Dine In'){
			if($('#single_order_'+sale_id+' .items_holder .single_item[data-selected=selected]').length>0){
				...
				previous_id_array.forEach(function(entry) {
					...
					$('#detail_item_id_'+entry+' .single_item_right_side .single_item_cooking_status').html('Preparing');
					

				});
				if(previous_id!=''){
					var url = base_url+"Bar/update_cooking_status_ajax";
					$.ajax({
						url:url,
						method:"POST",
						data:{
			...
			if(previous_id!=''){
				var url = base_url+"Bar/update_cooking_status_delivery_take_away_ajax";
				$.ajax({
					url:url,
					method:"POST",
					data:{
		...
		if($('#single_order_'+sale_id).attr('data-order-type')=='Dine In'){
			if($('#single_order_'+sale_id+' .items_holder .single_item[data-selected=selected]').length>0){
				...
				previous_id_array.forEach(function(entry) {
					...
					$('#detail_item_id_'+entry).removeClass('light-sky-background');
					$('#detail_item_id_'+entry+' .single_item_right_side .single_item_cooking_status').css('color','#fff');
					$('#detail_item_id_'+entry+' .single_item_right_side .single_item_cooking_status').html('Done');
				});
				if(previous_id!=''){
					var url = base_url+"Bar/update_cooking_status_ajax";
					$.ajax({
			...
			for (var key in response) {
				...
				notifications_list += '<div class="single_row_notification fix" id="single_notification_row_'+this_notification.id+'">';
					...
					notifications_list += '<div class="fix single_serve_button">';
						notifications_list += '<button class="single_serve_b" id="notification_serve_button_'+this_notification.id+'">Delete</button>';
					notifications_list += '</div>';
				notifications_list += '</div>';
			}
...
function refresh_orders() {
	var url = base_url+"Bar/get_new_orders_ajax"; 
	$('#refresh_it_or_not').html('Yes');
	$.ajax({
		url:url,
		method:"POST",
		...
		success:function(response) {
					...
					$order_list_left += '<div class="fix floatleft single_order" data-order-type="'+order_type+'" data-selected="'+selected_unselected+'" id="single_order_'+response[key].sales_id+'">';
	                    $order_list_left += '<div class="header_portion light-blue-background fix">';
	                        $order_list_left += '<div class="fix floatleft half">';
	                            $order_list_left += '<p class="order_number">Invoice: '+order_name+'</p> ';
	                            $order_list_left += '<p class="order_number">Table: '+tables_booked+'</p> ';
	                        $order_list_left += '</div>';
	                        $order_list_left += '<div class="fix floatleft half">';
	                            $order_list_left += '<p class="order_duration dark-blue-background"><span id="bar_time_minute_'+response[key].sales_id+'">'+minute+'</span>:<span id="bar_time_second_'+response[key].sales_id+'">'+second+'</span></p>';
	                        $order_list_left += '</div>';
	                    $order_list_left += '</div>';
	                    $order_list_left += '<div class="fix items_holder">';
	                        ...
	                        for (var key_item in items) {
	                            ...
	                            $order_list_left += '<div data-selected="unselected" class="fix single_item '+item_background+'" data-order-id="'+response[key].sales_id+'" data-item-id="'+single_item.previous_id+'" id="detail_item_id_'+single_item.previous_id+'" data-cooking-status="'+single_item.cooking_status+'">';
	                                $order_list_left += '<div class="single_item_left_side fix">';
		                                $order_list_left += '<div class="fix floatleft item_quantity">';
		                                    $order_list_left += '<p class="item_quanity_text" style="'+font_style+'">'+single_item.qty+'</p>';
		                                $order_list_left += '</div>';
		                                $order_list_left += '<div class="fix floatleft item_detail">';
		                                    $order_list_left += '<p class="item_name" style="'+font_style+'">'+single_item.menu_name+'</p>';
		                                    $order_list_left += '<p class="item_qty" style="font-weight:bold; '+font_style+'">Qty: '+single_item.qty+'</p>';

		                                    var modifiers = single_item.modifiers;
		                                    modifiers_length = modifiers.length;
		                                    var w = 1;
		                                    var modifiers_name = '';
		                                    for(var key_modifier in modifiers){
	                                ...
	                                $order_list_left += '<div class="single_item_right_side fix">';
		                                $order_list_left += '<p class="single_item_cooking_status" style="'+font_style+'">'+cooking_status+'</p>';
		                            $order_list_left += '</div>';
	                            $order_list_left += '</div>';
	                        }
			...
			*******************************************************************************************************************************************************************/
            var group_order_by_item_option = '<select id="group_by_order_item" class="group_by_order_item">';
            group_order_by_item_option += '<option value="">Select Item</option>';
            for (var key in window.order_items) {
                var single_ordered_item = window.order_items[key];
                group_order_by_item_option += '<option value="'+single_ordered_item.food_menu_id+'">'+single_ordered_item.menu_name+'</option>';
            }
