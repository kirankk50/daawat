...
$(document).ready(function(){
	...
	$(document).on('click','.items_holder .single_item',function(){
		...
		if(single_order.attr('data-order-type')=="Dine In"){
			if($(this).attr('data-selected')=="selected"){
				...
				if($(this).attr('data-cooking-status')=="Done" || $(this).attr('data-cooking-status')=="Started Cooking"){
	...
	$(document).on('click','.select_all_of_an_order',function(){
		...
		$('#single_order_'+order_id+' .items_holder .single_item').find('.single_item_cooking_status').css('color','#fff');
		$('#single_order_'+order_id+' .items_holder .single_item').attr('data-selected','selected');
	...
	$(document).on('click','.unselect_all_of_an_order',function(){
		...
		$('#single_order_'+order_id+' .items_holder .single_item').find('.single_item_cooking_status').css('color','');
	...
	$(document).on('click','.start_cooking_button',function(){
		...
		if($('#single_order_'+sale_id).attr('data-order-type')=='Dine In'){
			if($('#single_order_'+sale_id+' .items_holder .single_item[data-selected=selected]').length>0){
				...
				if(previous_id!=''){
					var url = base_url+"Kitchen/update_cooking_status_ajax";
					$.ajax({
						url:url,
						method:"POST",
						data:{
			...
			if(previous_id!=''){
				var url = base_url+"Kitchen/update_cooking_status_delivery_take_away_ajax";
				$.ajax({
					url:url,
					method:"POST",
					data:{
		...
		if($('#single_order_'+sale_id).attr('data-order-type')=='Dine In'){
			if($('#single_order_'+sale_id+' .items_holder .single_item[data-selected=selected]').length>0){
				...
				previous_id_array.forEach(function(entry) {
					...
					$('#detail_item_id_'+entry).removeClass('light-sky-background');
					$('#detail_item_id_'+entry+' .single_item_right_side .single_item_cooking_status').css('color','#fff');
					$('#detail_item_id_'+entry+' .single_item_right_side .single_item_cooking_status').html('Ready');
				});
				if(previous_id!=''){
					var url = base_url+"Kitchen/update_cooking_status_ajax";
					$.ajax({
	...
	$('#start_cooking').on('click',function(){
		if($('#order_details_holder .single_order[data-selected=selected]').attr('data-order-type')=='Dine In'){
			...
			if(previous_id!=''){
				$.ajax({
					url:base_url+"Kitchen/update_cooking_status_delivery_take_away_ajax",
					method:"POST",
					data:{
						previous_id : previous_id,
						cooking_status : 'Started Cooking',
					...
					$order_list_left += '<div class="fix floatleft single_order" data-order-type="'+order_type+'" data-selected="'+selected_unselected+'" id="single_order_'+response[key].sales_id+'">';
	                    $order_list_left += '<div class="header_portion light-blue-background fix">';
	                        ...
	                        $order_list_left += '<div class="fix floatleft" style="width:30%;">';
	                            $order_list_left += '<p class="order_duration dark-blue-background"><span id="kitchen_time_minute_'+response[key].sales_id+'">'+minute+'</span>:<span id="kitchen_time_second_'+response[key].sales_id+'">'+second+'</span></p>';
	                        $order_list_left += '</div>';
	                    $order_list_left += '</div>';
